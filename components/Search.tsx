...
interface SearchProps {
  ...
  setGlobalPlaylistId: React.Dispatch<React.SetStateAction<string>>;
...
const Search: React.FC<SearchProps> = ({
  ...
  async function updateSearchResults(query: string) {
    const response = await fetch(
      "https://api.spotify.com/v1/search?" + new URLSearchParams({ q: query, type: ["artist", "playlist", "track"] } as any),
      {
        headers: {
